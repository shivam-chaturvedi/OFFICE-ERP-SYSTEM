...
const ManageDepartments = () => {
  ...
  const handleAddDepartment = (newDept) => {
    ...
    setDepartments(prev => [...prev, { id: newId, created, employees: 0, projects: 0, ...newDept }]);
  };
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-3xl font-bold mt-2 ${color === 'green' ? 'text-green-600' : color === 'red' ? 'text-red-600' : 'text-gray-900'}`}>
            {value}
          </p>
        </div>
        <div className={`p-3 rounded-full ${color === 'green' ? 'bg-green-100' : color === 'red' ? 'bg-red-100' : 'bg-gray-100'}`}>
          <Icon className={`w-6 h-6 ${color === 'green' ? 'text-green-600' : color === 'red' ? 'text-red-600' : 'text-gray-600'}`} />
        </div>
      </div>
      {trend && (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Organize and manage your company departments</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Total Departments" value={totalDepartments} icon={Building2} />
          <StatCard title="Active Departments" value={activeDepartments} icon={TrendingUp} color="green" trend="up" />
          <StatCard title="Inactive Departments" value={inactiveDepartments} icon={TrendingDown} color="red" trend="down" />
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Department ID</th>
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Employees</th>
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Department Head</th>
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Created</th>
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Status</th>
                  <th className="text-left py-3 px-6 font-medium text-gray-900">Projects</th>
                  ...
                  <tr key={dept.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                          dept.head === 'Not assigned' ? 'bg-gray-200 text-gray-500' : 'bg-green-100 text-green-700'
                        }`}>
                          {dept.head === 'Not assigned' ? '?' : dept.head.charAt(0)}
                        </div>
                        <span className={dept.head === 'Not assigned' ? 'text-gray-500' : 'text-gray-900'}>
                          {dept.head}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      ...
                      <span className={`ml-2 text-sm font-medium ${
                        dept.status === 'Active' ? 'text-green-600' : 'text-gray-500'
                      }`}>
                        {dept.status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-gray-900 font-medium">{dept.projects}</td>
                    <td className="py-4 px-6">
        ...
        {filteredDepartments.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-500">No departments found matching your search criteria.</div>
          </div>
        )}

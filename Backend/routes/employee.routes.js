const express = require("express");
const router = express.Router();
const employeeController = require("../controllers/employee.controller");
const authMiddleware = require("../middlewares/auth.middleware");

router.post("/add", authMiddleware.verifyHrOrAdmin, employeeController.addEmployee);
router.post("/apply-leave", employeeController.applyLeave);
router.get("/leaves/:id", employeeController.getAllLeaves);
router.get("/", authMiddleware.verifyHrOrAdmin, employeeController.getAllEmployees);
router.get("/:id", employeeController.getEmployee);
router.delete("/leave/delete/:id", employeeController.withdrawLeave);

router.put(
  "/edit",
  authMiddleware.verifyHrOrAdmin,
  employeeController.editEmployee
);
module.exports = router;
